...
# A simulated survey, used as a realistic design: 150 sites along a depth
# gradient, sampled in two periods. Fixed seed, so it is the same data set
# every time the app starts.
#
# The counts really were generated by a Poisson GLM with a log link, so the
# students can recover these values with the sliders:
#
...
SURVEY <- local({
  set.seed(112)
  n     <- 150
  depth <- round(runif(n, 0.8, 5), 3)
  per   <- rep(c(1, 2), times = c(100, 50))
  eta   <- 5 - 0.6 * depth + 0.4 * (per == 2)
  data.frame(depth_km  = depth,
             abundance = rpois(n, exp(eta)),
             period    = per)
})
    ...
    sidebarPanel(
      ...
      conditionalPanel(
        ...
        tags$p(style = "color:#777; font-size:90%;",
               "150 sites along a depth gradient, sampled in two periods.",
