...
# A simulated survey, used as a realistic design: 150 sites at five depths,
# 30 sites per depth (20 in period 1, 10 in period 2). Fixed seed, so it is
# the same data set every time the app starts.
#
# Whole-number depths on purpose: the students can work out eta_i in their
# head and check it against the table.
#
# The counts really were generated by a Poisson GLM with a log link, so these
# values can be recovered with the inputs:
#
#     beta_1 = 5, beta_2 = -0.6, beta_3 = 0.4   (period 2 vs period 1)
#
# Real count data are almost always overdispersed, which makes them useless
# for a "tune the betas until it fits" exercise. This one is not.
SURVEY <- local({
  set.seed(112)
  depth  <- rep(1:5, each = 30)
  period <- rep(rep(c(1, 2), times = c(20, 10)), times = 5)
  eta    <- 5 - 0.6 * depth + 0.4 * (period == 2)
  data.frame(depth_km  = depth,
             abundance = rpois(length(depth), exp(eta)),
             period    = period)
})
...
# Same thing, but the design comes from the survey: one row per sampled site,
# x_i2 = the depth at which it was sampled, and the factor = the period.
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      checkboxInput("use_data",
                    "Use the survey as the design", value = FALSE),
      conditionalPanel(
        "input.use_data == true",
        tags$p(style = "color:#777; font-size:90%;",
               "150 sites at five depths, sampled in two periods.",
               "\\(x_{i2}\\) is now the depth (km) of each site and the factor is",
               "the period. The counts are drawn as black crosses. Can you find",
               "the \\(\\beta_j\\) they were generated with?")
      ),
      conditionalPanel(
...
server <- function(input, output, session) {
  ...
  x_values <- reactive({
    if (isTRUE(input$use_data)) return(sort(unique(SURVEY$depth_km)))
    lo <- input$x_min
    hi <- input$x_max
  ...
  # How wide a distribution may be drawn: the gap between neighbouring x values.
  spacing <- reactive({
    xv <- x_values()
  ...
  sim <- reactive({
    d <- if (isTRUE(input$use_data)) {
      build_design_from_data(SURVEY, isTRUE(input$use_factor), input$n_rep)
    } else {
  ...
  output$plot_note <- renderUI({
    ...
    if (isTRUE(input$use_data))
      bits <- c(bits, paste("Black crosses are the counts observed at those depths.",
                            "Try to choose the \\(\\beta_j\\) so that the simulated",
                            "cloud looks like the observed one."))
